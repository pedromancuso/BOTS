...
#include <WebServer.h>
#include <WiFi.h>
#include <Esp32WifiManager.h>/*#include <ESP8266WiFi.h>*/
#include <ESP32httpUpdate.h>/*#include <ESP8266HTTPClient.h> /*Poner esta libreria despues de ESP8266WiFi.h*/
  ...
  if(WiFi.status() == WL_CONNECTED){
    if(BOTSCONNECTED==true){
      ...
      if(MQTTCONNECTED==true){
        if(client.connected()){digitalWrite(LED_PIN,LOW);}else{digitalWrite(LED_PIN,HIGH);}
        mqttclienthandle();
