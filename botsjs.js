...
function selectday(a){
	...
	if (ID(a).value==false){ID(a).className = 'button special fit small disabled';}
	if (ID(a).value==true){ID(a).className = 'button special fit small';}
}
function delmenu() {
	var cols = document.getElementsByClassName('deleteop');
	for(i = 0; i < cols.length; i++) {
		if (cols[i].style.display=='none'){cols[i].style.display='table-cell';ID('eliminarprogramas').style.display='table-cell';}
...
function programeditshow(IDx){
	if (IDx==0) {
		...
		for (var i = 1; i <= 7; i++) {ID('D'+i).value=false;ID('D'+i).className='button special fit small disabled';}
		ID('HORAINICIO').value='HH';
		ID('MINUTOINICIO').value='MM';
		ID('HORAFIN').value='HH';
		ID('MINUTOFIN').value='MM';
		...
		for (var i = 0; i < programasres.length; i++) {
			if(programasres[i][0]==ID){
				...
				ID('MINUTOINICIO').value=inicio[1];
				var fin = programasres[i][5].split(':');
				ID('HORAFIN').value=fin[0];
				ID('MINUTOFIN').value=fin[1];
	...
	ID('tabladevices').style.height=(((ID('programbutton').getBoundingClientRect().top)-(ID('HMF').getBoundingClientRect().bottom))*0.95)+'px';
}
...
function user(){
	...
	var footerHeight=ID('footerx').getBoundingClientRect().height;
	...
	for (var i = 0; i <= 60; i++) {
		...
		if (i<=24) {ID('HORAINICIO').innerHTML+=line;ID('HORAFIN').innerHTML+=line;}
		ID('MINUTOINICIO').innerHTML+=line;ID('MINUTOFIN').innerHTML+=line;
	}
	/*mqtt();*/
}
